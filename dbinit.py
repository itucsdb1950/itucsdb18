...
INIT_STATEMENTS = [
	...
	"CREATE TABLE CLASS (crn INTEGER(5), course_code VARCHAR(7), loc_id INTEGER(5) FOREIGN KEY REFERENCES LOCATION(loc_id), credit INTEGER(1));",
	"CREATE TABLE LOCATION (class VARCHAR(10), building CHAR(3), day CHAR(3), start_time TIME(), end_time TIME(), year YEAR, loc_id INTEGER(5) PRIMARY KEY);",
	"CREATE TABLE LOCATION (dep_id INTEGER(4), dep_name VARCHAR(30), fac_name VARCHAR(30), dean_id INTEGER FOREIGN KEY REFERENCES PERSON(id), stu_delegate INTEGER FOREIGN KEY REFERENCES PERSON(id));"
]
